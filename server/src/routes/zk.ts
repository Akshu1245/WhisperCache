...
// ============================================================================
// MIDNIGHT INTEGRATION ROUTES (Simplified for tests)
// ============================================================================

// Use ProofStore from in-memory db for storing proofs
const proofs = inMemoryDb.ProofStore;
...
router.post('/midnight/generate-witness', async (req: Request, res: Response) => {
  try {
    const { query, memoryCategory, userId } = req.body || {};
 ...
 * Generate a Midnight Compact proof (simulated, non-blocking)
 */
router.post('/midnight/generate-proof', async (req: Request, res: Response) => {
  try {
    ...
    proofs.set(hash, { proof, storedAt: Date.now() });
...
router.post('/midnight/verify-proof', async (req: Request, res: Response) => {
  try {
    ...
    // proofData is required; witness is also expected but we mainly need proofData
    if (!proofData) {
    ...
    try {
      ...
      return res.status(400).json({ error: 'malformed proofData' });
    }
...
router.post('/midnight/export-for-anchoring', async (req: Request, res: Response) => {
  try {
    ...
    const entry = proofs.get(proofHash);
    if (!entry) {
...
router.post('/midnight/cli-demo', async (req: Request, res: Response) => {
  try {
    const { query, demoMode = false } = req.body || {};
